import {
  ...
  function handleFiles() {
    const uploadFile = document.getElementById('fileInput').files;
    ...
    if (uploadFile)
    {
      console.log("attempting to write file")
            for (var i = 0; i < uploadFile.length; i++) {
              writeFileToPod(uploadFile[i], `${SOLID_IDENTITY_PROVIDER}/dominique/uploadedFiles/${uploadFile[i].name}`, session.fetch);
          }
    }
  }
  ...
  async function writeFileToPod(file, targetFileURL, fetch ) {
    try {
      ...
      readFileFromPod(getSourceUrl(savedFile));
  ...
  async function readFileFromPod(fileURL) {
    try {
      ...
      var profImage = URL.createObjectURL(file)
      console.log( `Fetched a ${getContentType(file)} file from ${getSourceUrl(file)}.`);
      console.log(`The file is ${isRawData(file) ? "not " : ""}a dataset.`);
      document.getElementById("profilePic").src = profImage;
